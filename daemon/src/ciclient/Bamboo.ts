...
const BambooResultDecoder: Decoder<BambooResult> = D.mapObject(
  D.mapRequiredFields({
    buildState: D.str,
...
const BambooResponseDecoder: Decoder<BambooResult> = D.at(
  ["results", "result"],
  D.andThen((resArray: BambooResult[]) => {
...
export class BambooFetch extends Fetch<BambooConfig> {
  ...
  constructor(
    uuid: string,
    config: BambooConfig,
    onResult: (status: BuildStatus) => void
  ) {
    super(uuid, config, onResult);

    const { serverUrl, plan, token } = config;
    const planPart = serverUrl + "/rest/api/latest/result/" + plan + ".json";

    const url = planPart + "?max-results=1&expand=results.result";
...
export const BambooConfigDecoder: Decoder<BambooConfig> = D.mapObject({
  ...D.mapRequiredFields({
    serverUrl: D.str,
    plan: D.str,
  }),
